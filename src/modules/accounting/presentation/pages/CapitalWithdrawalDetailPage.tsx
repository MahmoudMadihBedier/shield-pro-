/**
 * One capital withdrawal: envelope, the Draft→Submit / Submitted→Cancel bar,
 * and — once submitted — the action that posts it to the general ledger
 * (`Dr owners_drawings / Cr <source_account>`, idempotent by `voucher_no`).
 */
...
import { postWithdrawalToGl, reverseGlPosting, type GlPosting } from '../../data/post-accounting'
...
export function CapitalWithdrawalDetailPage() {
  const { id = '' } = useParams()
  const navigate = useNavigate()
  const perms = useAccountingPermissions()

  const query = useCapitalWithdrawal(id)
  const { createDraft, submit, cancel } = useCapitalWithdrawalActions()
  ...
  async function handleAmend(row: CapitalWithdrawal, reason: string) {
    ...
    try {
      ...
      navigate(`/accounting/capital-withdrawals/${amended.$id}`)
    } catch (e) {
  ...
  const postGlMutation = useMutation<GlPosting, AppError, CapitalWithdrawal>({
    mutationFn: async (row) => {
      const result = await postWithdrawalToGl(row)
  ...
  return (
    <div className="space-y-4">
      ...
      <PageHeader
        title={`سحب رأس مال ${row?.reference_id ?? ''}`}
        titleEn="Capital withdrawal"
        actions={
        ...
        <>
          <Card className="space-y-2 text-sm">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge tone="neutral">{CAPITAL_WITHDRAWAL_METHOD_LABELS[row.method].ar}</Badge>
            ...
            <dl className="grid gap-x-6 gap-y-1 sm:grid-cols-2">
              <div className="flex justify-between">
                <dt className="text-zinc-500">المستفيد / Withdrawn by</dt>
                <dd>{row.withdrawn_by}</dd>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <dt className="text-zinc-500">حساب المصدر / Source account</dt>
                <dd dir="ltr">{row.source_account}</dd>
              </div>
            </dl>
            {row.reason ? (
          ...
          <Dialog
            open={editOpen}
            title="تعديل سحب رأس مال"
            titleEn="Edit capital withdrawal"
